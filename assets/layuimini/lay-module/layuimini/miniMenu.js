...
layui.define(["element", "jquery"], function (exports) {
    ...
    var miniMenu = {
        ...
        render: function (options) {
            ...
            $('.layui-nav .layui-nav-item a span').hide()
            element.init();
        },
        ...
        renderSingleModule: function (menuList, menuChildOpen) {
            ...
            $.each(menuList, function (index, menu) {
                ...
                if(menu.icon){
                    var icons = '<i class="iconfont">&#' + menu.icon + ';</i>';
                }else{
                    var icons = '<i class="menuzef"></i>';
                }
                if (menu.child != undefined && menu.child != []) {
                    
                    leftMenuHtml += '<a href="javascript:;" class="layui-menu-tips" >' + icons + '<span class="layui-left-nav"> ' + menu.title + '</span> </a>';
                    var buildChildHtml = function (html, child) {
                        ...
                        $.each(child, function (childIndex, childMenu) {
                            ...
                            if (childMenu.child != undefined && childMenu.child != []) {
                                if(childMenu.icon){
                                    var childIcons = '<i class="iconfont">&#' + childMenu.icon + ';</i>';
                                }else{
                                    var childIcons = '<i class="menuzef"></i>'
                                }
                                html += '<a href="javascript:;" class="layui-menu-tips" >' + childIcons + '<span class="layui-left-nav"> ' + childMenu.title + '</span></a>';
                                html = buildChildHtml(html, childMenu.child);
                            } else {
                                html += '<a href="javascript:;" class="layui-menu-tips"  layuimini-href="' + childMenu.href + '" target="' + childMenu.target + '">' + childIcons + '<span class="layui-left-nav"> ' + childMenu.title + '</span></a>\n';
                            }
                            html += '</dd>\n';
                        });
                        html += '</dl>\n';
                    ...
                    leftMenuHtml += '<a href="javascript:;" class="layui-menu-tips"  layuimini-href="' + menu.href + '" target="' + menu.target + '">' + icons + '<span class="layui-left-nav"> ' + menu.title + '</span></a>\n';
                }
                leftMenuHtml += '</li>\n';
            });
            leftMenuHtml += '</ul>\n';
        ...
        listen: function () {
            ...
            $('body').on('click', '[data-side-fold],.layuimini-site-mobile', function () {
                var loading = layer.load(0, {shade: false, time: 200});
            ...
            element.on('nav()', function(data){
                if(!$(this).hasClass('layui-menu-tips')) return
                var loading = layer.load(0, {shade: false, time: 2 * 1000});
                var isShow = $('.layuimini-tool [data-side-fold]').attr('data-side-fold');
                if(isShow != 1){
                    $('.layuimini-tool [data-side-fold]').attr('data-side-fold', 1);
                    ...
                    $('.layui-layout-body').removeClass('layuimini-mini');
                    $('.layui-layout-body').addClass('layuimini-all');
                    $('.layui-nav-item').find('span').css('display','inline-block');
                }
                element.init();
                layer.closeAll();
            });
